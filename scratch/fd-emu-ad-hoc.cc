 ...
 */
  ...
  * udp packet transmission on hardware via an ethernet cable
  * udp packet transmission on hardware when two nodes are on the same wifi network
  * Bash scripts to set up, turn on and turn off wifi hotspot

  * What has not been completed:
  * connecting two nodes on the same hotspot and sending data instead of them already being
  * on the same network
  */
...
main (int argc, char *argv[])
{
  ...
  /* Starting a WiFi hotspot with ssid 'up-squared'
   * Creates a WiFi hotspot but doesn't turn it on
   * Turn hotspot ON - if node is a server
   * Do not turn hotspot ON - if node is a client
   * Bash source code present in /home/santosh/bash_scripts
   */
  system("/home/tyjo/bash_scripts/set_up_hotspot.sh");
  ...
  ipv4.SetBase ("192.168.43.0", "255.255.255.0");

  /* Code to assign IP and MAC addresses to the client and server nodes 
   * client - node 0, IP - 192.168.43.2  
   * server - node 1, IP - 192.168.43.1 
   * The server and client should have the first and second IP address on the network respectively
   */
  if (clientMode)
    {
      ...
      dev->SetAddress (Mac48Address ("9c:b6:d0:dc:d2:b5")); //
      NS_LOG_INFO ("Assign IP Addresses.");
      ipv4.NewAddress ();  // burn the 192.168.43.1 address so that 192.168.43.2 is next
      i = ipv4.Assign (d);
    }
  else if (serverMode)
    {
      /* Turn hotspot ON (For Adhoc network. Not yet complete)*/
      system("/home/tyjo/bash_scripts/turn_on_hotspot.sh");
  ...
  /* Code to set up a UDP client or server application depending on whether the node is a client
   * or a server (decided by command line arguments) 
   */  
  ...
  if (serverMode)
  ...
  else if (clientMode)
    {
      ...
      UdpEchoClientHelper client (Ipv4Address ("192.168.43.1"), 9);
  ...
  emu.EnablePcapAll ("fd-emu-ad-hoc", true);
  emu.EnableAsciiAll ("fd-emu-ad-hoc.tr");
